...
def get_dataframes(
    ...
    dataframes = rc.convert_rosbags(
        rosbags_dir, typestore, keywords=keywords, verbose=verbose, topics=topics
    )
        ...
        for df in bag.values():
            if "header.stamp.sec" in df.columns:
                df["timestamp"] = pd.to_datetime(
                    df["header.stamp.sec"] * 1_000_000_000 + df["header.stamp.nanosec"],
                    unit="ns",
                )
...
def get_topic(bag: dict[str, pd.DataFrame], topic_name: str, ns=None):
    if ns is None:
        ...
        if (ns + topic_name) in bag.keys():
            return bag[ns + topic_name]
        else:
            return None
    return None
...
def plot_pose_w_cov(
    ...
    **kwargs,
    ...
    x_dist = (
        pose_df["pose.pose.position.x"].max() - pose_df["pose.pose.position.x"].min()
    )
    y_dist = (
        pose_df["pose.pose.position.y"].max() - pose_df["pose.pose.position.y"].min()
    )
    dist = 0.04 * max(
        x_dist, y_dist
    )  # Used to calculate the length of orientation lines
