...
def plot_waypoints(waypoints, radii, ax):
    ...
    for x, y in zip(x_waypoints, y_waypoints):
        ...
        inner_circle = Circle(
            (x, y), radius=capture_radius, color="green", fill=True, alpha=0.3
        )
        ax.add_patch(inner_circle)

        # Red outer circle with radius 10
        outer_circle = Circle(
            (x, y), radius=slip_radius, color="red", fill=False, linewidth=2
        )
...
def run(args):
    ...
    zoomLevel, pictureRadius = cordinatehandling.DetermineMinimumZoomLevel(
        centerCords[0], maxDistance
    )
    ...
    if args.location == "ul":
        ...
        mapAtLocation = gmaps.static_map(
            size=config.DEFAULT_OUTPUT_IMAGE_SIZE,
            center=cordinatehandling.CordToString(centerCords),
            maptype=mapType,
            zoom=zoomLevel,
        )
