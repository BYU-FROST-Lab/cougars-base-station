    ...
    with open(file_path, "r") as file:
        for line in file:
            ...
            if line.startswith("points ="):
                ...
                waypoints = [
                    tuple(map(float, point.split(","))) for point in point_pairs
                ]
...
def find_db3_file(directory):
    ...
    if len(db3_files) == 1:
        ...
        raise ValueError(
            f"Multiple .db3 files found in directory: {directory}. Please specify one."
        )
...
def run(args):
    ...
    if args.location == "ul":
        ...
        ref_lat, ref_lon = cordinatehandling.FindCenter(
            [curr.gps_lat for curr in data_dict[config.INTERPRETED_GPS]],
            [curr.gps_long for curr in data_dict[config.INTERPRETED_GPS]],
        )
