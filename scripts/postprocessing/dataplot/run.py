...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        "--latlongaxis",
        action="store_true",
        help="In mapping, use latitude / logitude coordinates for the axis instead of meters",
    )
    parser.add_argument(
        "-P",
        "--process",
        choices=["csv", "dat", "map", "all", "rosbag"],
        default="all",
        help="Specifiy which of the subprocesses will run",
    )
    parser.add_argument(
        "-c",
        "--colormap",
        choices=config.COLORMAP_OPTIONS,
        default="jet_r",
        help="The colormap used for the plotting",
    )
    parser.add_argument(
        "-s",
        "--samplereduction",
        type=int,
        default=1,
        help="The number by which to reduce the data. For example, if n is input, only (1/n)th of the data will be processed and mapped.",
    )
    parser.add_argument(
        "-l",
        "--location",
        choices=["ul", "byu", "online"],
        default="online",
        help="Specifiy which map location. Defaults to online map query",
    )
    parser.add_argument(
        "-b",
        "--behavior",
        action="store_true",
        help="a .bhv file is in directory containing the waypoint data",
    )
