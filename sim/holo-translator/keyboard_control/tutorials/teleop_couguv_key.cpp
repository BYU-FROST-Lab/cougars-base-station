...
private:
  ...
  rclcpp::Publisher<holoocean_interfaces::msg::ControlCommand>::SharedPtr
      command_pub_;
...
TeleopCommand::TeleopCommand()
  ...
  command_pub_ = create_publisher<holoocean_interfaces::msg::ControlCommand>(
      "holoocean/command/control", 10);
  ...
  last_command_msg_.cs = {fin1_ * M_PI / 180.0, fin2_ * M_PI / 180.0,
                          fin3_ * M_PI / 180.0,
                          static_cast<double>(thruster_value_)};

  timer_ = create_wall_timer(std::chrono::milliseconds(1500),
