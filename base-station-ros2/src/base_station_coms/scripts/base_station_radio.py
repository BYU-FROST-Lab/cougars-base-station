...
class RFBridge(Node):
    ...
    def recieve_status(self, data):
        ...
        status.dvl_vel.velocity = dvl_pos.get('v', 0.0)
        status.imu_data.orientation.x = imu_data.get('x', 0.0)
        status.imu_data.orientation.y = imu_data.get('y', 0.0)
        status.imu_data.orientation.z = imu_data.get('z', 0.0)
        status.imu_data.orientation.w = imu_data.get('w', 1.0)
