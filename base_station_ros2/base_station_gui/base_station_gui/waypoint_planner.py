...
class App:
    def __init__(self, root_widget):
        ...
        self.waypoints = (
            []
        )  # List of {'east': float, 'north': float, 'up': float, 'depth': float,
        ...
        ttk.Label(
            self.control_frame, text="Mission Origin", font=("Arial", 14, "bold")
        ).pack(pady=(0, 5))
        self.set_origin_button = ttk.Button(
            self.control_frame, text="Set Origin", command=self.prompt_origin_choice
        )
        self.set_origin_button.pack(pady=5, fill=tkinter.X)
        self.origin_info_label = ttk.Label(
            self.control_frame, text="Origin not set.", wraplength=320
        )  # Increased wraplength
        self.origin_info_label.pack(pady=5)

        ttk.Label(
            self.control_frame,
            text="Waypoints (ENU relative to Origin)",
            font=("Arial", 14, "bold"),
        ).pack(pady=(10, 5))
        self.waypoints_listbox = tkinter.Listbox(
            self.control_frame, height=15, width=50
        )  # Increased width
        self.waypoints_listbox.pack(pady=5, fill=tkinter.BOTH, expand=True)

        self.clear_waypoints_button = ttk.Button(
            self.control_frame,
            text="Clear All Waypoints & Origin",
            command=self.clear_mission_confirmation,
        )
        self.clear_waypoints_button.pack(pady=5, fill=tkinter.X)

        ttk.Label(
            self.control_frame, text="Mission Actions", font=("Arial", 14, "bold")
        ).pack(pady=(10, 5))
        self.save_mission_button = ttk.Button(
            self.control_frame,
            text="Save Mission to .yaml File",
            command=self.save_mission_to_yaml,
        )
        ...
        self.map_widget = tkintermapview.TkinterMapView(
            self.map_frame, width=700, height=700, corner_radius=0
        )
    ...
    def _geodetic_to_enu(
        self, lat_wp_deg, lon_wp_deg, alt_wp, lat_origin_deg, lon_origin_deg, alt_origin
    ):
        """
        ...
        x_origin_ecef, y_origin_ecef, z_origin_ecef = lla_to_ecef(
            lat_origin_rad, lon_origin_rad, alt_origin
        )
        ...
        north = (
            -sin_lat_o * cos_lon_o * dx - sin_lat_o * sin_lon_o * dy + cos_lat_o * dz
        )
    ...
    def _update_ui_state(self):
        if self.origin_data:
            ...
            self.clear_waypoints_button.config(
                state=tkinter.NORMAL
            )  # Enable clearing if origin or waypoints exist
        else:
            self.save_mission_button.config(state=tkinter.DISABLED)
            if (
                not self.waypoints
            ):  # Only disable clear if both origin and waypoints are empty
    ...
    def prompt_origin_choice(self):
        ...
        for option_text in options:
            rb = ttk.Radiobutton(
                dialog, text=option_text, variable=var, value=option_text
            )
        ...
        def on_ok():
            ...
            if choice == "Manual Entry":
                ...
                self._set_origin(
                    preset_data["lat"], preset_data["lon"], preset_data["alt"], choice
                )
    ...
    def prompt_manual_origin(self):
        ...
        try:
            ...
            tkinter.messagebox.showerror(
                "Invalid Input",
                f"Please enter valid numbers for Lat, Lon, Alt.\nError: {e}",
            )
            return
    ...
    def _set_origin(self, lat, lon, alt, origin_name="Origin"):
        ...
        origin_map_marker = self.map_widget.set_marker(
            lat,
            lon,
            text=origin_marker_text,
            text_color="white",
        ...
        self.origin_info_label.config(
            text=f"Origin ({origin_name}):\nLat={lat:.6f}, Lon={lon:.6f}, Alt={alt:.2f}m"
        )
        ...
        tkinter.messagebox.showinfo(
            "Origin Set",
            f"Origin '{origin_name}' set successfully. Right-click map to add waypoints.",
        )
    ...
    def add_waypoint_on_map_click(self, coords):
        if not self.origin_data:
            tkinter.messagebox.showwarning(
                "Set Origin First",
                "Please set the mission origin before adding waypoints.",
            )
            return
        ...
        depth_str = tkinter.simpledialog.askstring(
            "Waypoint Depth",
            f"Enter Depth for Waypoint {wp_index} (meters, positive down):",
            parent=self.root_widget,
        )
        ...
        try:
            ...
            if depth < 0:
                tkinter.messagebox.showerror(
                    "Invalid Input", "Depth must be a non-negative number."
                )
                return
        except ValueError:
            tkinter.messagebox.showerror(
                "Invalid Input", "Please enter a valid number for depth."
            )
            return
        ...
        east, north, up_enu = self._geodetic_to_enu(
            clicked_lat,
            clicked_lon,
            origin_alt,  # wp alt = origin alt for this
            origin_lat,
            origin_lon,
            origin_alt,
        )

        map_marker = self.map_widget.set_marker(
            clicked_lat,
            clicked_lon,
            text=f"WP{wp_index}\nD:{depth:.1f}m",
            text_color="white",
    ...
    def _update_waypoint_display(self):
        ...
        for wp in self.waypoints:
            display_text = (
                f"WP {wp['id']}: X={wp['x']:.2f}, Y={wp['y']:.2f}, Z={wp['z']:.2f}m | D={wp['depth']:.1f}m "
    ...
    def clear_mission_confirmation(self):
        ...
        if tkinter.messagebox.askyesno(
            "Confirm Clear",
            "Clear current origin and all waypoints? This cannot be undone.",
        ):
    ...
    def clear_mission_data(self, show_confirmation=True):
        ...
        if show_confirmation:
            tkinter.messagebox.showinfo(
                "Cleared", "Mission data and waypoints cleared."
            )

    def save_mission_to_yaml(self):
        if not self.origin_data:
            tkinter.messagebox.showerror(
                "Error", "Cannot save mission. Origin not set."
            )
            return
        if not self.waypoints:
            tkinter.messagebox.showwarning(
                "No Waypoints", "Mission has an origin but no waypoints. Save anyway?"
            )
        ...
        mission_name = (
            base_name.lower().replace("_mission.yaml", "").replace(".yaml", "")
        )
            ...
            mission_content["waypoints"].append(
                {
                    ...
                    "position_enu": {  # Relative to origin_lla
                        ...
                        "z": round(
                            wp["z"], 3
                        ),  # meters (height relative to origin's altitude)
                    },
                    "depth": round(
                        wp["depth"], 2
                    ),  # meters (positive downwards from waypoint's ENU position)
        ...
        try:
            ...
            tkinter.messagebox.showinfo(
                "Success", f"Mission saved to {os.path.basename(filepath)}"
            )
        except Exception as e:
            tkinter.messagebox.showerror(
                "File Save Error", f"Could not save mission YAML file: {e}"
            )
