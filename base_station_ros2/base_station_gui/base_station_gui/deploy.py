...
def load_config(sel_vehicles):
    ...
    for num in sel_vehicles:
        if str(num) in vehicles:
            ...
            ros_node.console_log.publish(
                ConsoleLog(
                    message=f"❌ Vehicle {num} not found in config, consider adding (skipping)",
                    vehicle_number=num,
                )
            )
    return result
...
def sftp_file(
    file_path, remote_user, remote_host, remote_path, remote_filename, vehicle_num
):
    """Deletes existing file then copies a new one via SFTP using SSH key authentication."""
    try:
        ros_node.console_log.publish(
            ConsoleLog(
                message=f"🗑️ Deleting {remote_filename} on {remote_host}...",
        ...
        if ssh is None:
            ros_node.console_log.publish(
                ConsoleLog(
                    message=f"❌ Failed to establish SSH connection to {remote_host}",
                    vehicle_number=vehicle_num,
                )
            )
            return False
        ...
        ros_node.console_log.publish(
            ConsoleLog(
                message=f"📤 Copying {file_path} to {remote_user}@{remote_host}:{remote_full_path}...",
                vehicle_number=vehicle_num,
            )
        )
        ...
        ros_node.console_log.publish(
            ConsoleLog(message=f"❌ SFTP error: {e}", vehicle_number=vehicle_num)
        )
        return False
...
def get_ssh_connection(ip_address, remote_user):
    try:
        ...
        ros_node.console_log.publish(
            ConsoleLog(
                message=f"SSH Authentication failed for {ip_address}. Attempting to copy SSH key. Enter password in the terminal",
                vehicle_number=0,
            )
        )
        try:
            if ensure_ssh_key():
                ...
                ros_node.console_log.publish(
                    ConsoleLog(
                        message=f"SSH key copied successfully to {ip_address}.",
                        vehicle_number=0,
                    )
                )
            ...
            ros_node.console_log.publish(
                ConsoleLog(
                    message=f"Failed to copy SSH key to {ip_address}: {e}",
                    vehicle_number=0,
                )
            )
    except Exception as e:
        ros_node.console_log.publish(
            ConsoleLog(
                message=f"Failed to connect to {ip_address}: {e}", vehicle_number=0
            )
        )
        return None
...
def log_deployment(vehicle, files_sent, vehicle_number):
    ...
    log_file = os.path.join(
        DEPLOY_HISTORY_DIR, f"{vehicle['name']}_deployment_{timestamp}.txt"
    )
    ...
    ros_node.console_log.publish(
        ConsoleLog(
            message=f"✅ Deployment logged: {log_file}\n", vehicle_number=vehicle_number
        )
    )
...
def main(passed_ros_node, sel_vehicles, passed_file_paths=[]):  # selected vehicles
    ...
    for i, vehicle in enumerate(vehicles):
        ...
        ros_node.console_log.publish(
            ConsoleLog(
                message=f"Mission path for coug{vehicle_number} was set as: {mission_path}",
                vehicle_number=vehicle_number,
            )
        )
        ...
        for label, file_path, remote_filename in [
            ...
            if os.path.exists(file_path):
                ...
                if success:
                    ...
                    ros_node.console_log.publish(
                        ConsoleLog(
                            message=f"❌ Failed to deploy {label} to {vehicle['name']}",
                            vehicle_number=vehicle_number,
                        )
                    )
                    load_success = False
                ...
                ros_node.console_log.publish(
                    ConsoleLog(
                        message=f"⚠️ File not found: {file_path} (skipping)",
                        vehicle_number=vehicle_number,
                    )
                )
                load_success = False
        ...
        message = (
            f"Coug{vehicle_number} mission loading finished with no errors."
            if load_success
            else f"Coug{vehicle_number} mission loading failed."
        )
        ros_node.console_log.publish(
            ConsoleLog(message=message, vehicle_number=vehicle_number)
        )
