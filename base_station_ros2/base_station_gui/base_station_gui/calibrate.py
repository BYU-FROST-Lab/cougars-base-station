...
def get_vehicle_config(vehicle_number):
    ...
    try:
        ...
        if vehicle_info:
            ...
            ros_node.publish_console_log(
                f"[ERROR] Vehicle {vehicle_number} not found in config", vehicle_number
            )
            return None
    except Exception as e:
        ros_node.publish_console_log(
            f"[ERROR] Failed to load config: {e}", vehicle_number
        )
        return None
...
def create_ssh_connection(vehicle_number):
    ...
    try:
        ...
        ros_node.publish_console_log(
            f"[ERROR] Failed to connect to vehicle {vehicle_number}: {e}",
            vehicle_number,
        )
        return None
...
def run_service_call(service_name, srv_type, args, vehicle):
    ...
    try:
        ...
        if "success=True" in output:
            ...
            ros_node.publish_console_log(
                f"[INFO] Service call to {service_name} succeeded. {match.group(1) if match else ''}",
                vehicle,
            )
        elif "success=False" in output:
            match = re.search(r"message=\s*(.*)", output)
            ros_node.publish_console_log(
                f"[WARNING] Service call to {service_name} failed with message: {match.group(1) if match else ''}",
                vehicle,
            )
        else:
            ros_node.publish_console_log(
                f"[ERROR] Unexpected response from service {service_name}: {output}",
                vehicle,
            )

    except Exception as e:
        ros_node.publish_console_log(
            f"[ERROR] Service call to {service_name} failed: {e}", vehicle
        )
    finally:
        ssh.close()
    ...
    try:
        ...
        def read_output():
            try:
                ...
                ros_node.publish_console_log(
                    f"[ERROR] Error reading output: {e}", vehicle
                )
        ...
        if exit_status != 0:
            ros_node.publish_console_log(
                f"[ERROR] Script {script_path} failed with exit code {exit_status}",
                vehicle,
            )
        else:
            ros_node.publish_console_log(
                f"[INFO] Script {script_path} completed successfully", vehicle
            )
        ...
        ros_node.publish_console_log(
            f"[ERROR] Failed to run script {script_path}: {e}", vehicle
        )
    finally:
        ssh.close()
...
def get_ros_param(namespace, vehicle):
    ...
    try:
        ...
        if error_output:
            ros_node.publish_console_log(
                f"[ERROR] Error getting parameter: {error_output}", vehicle
            )
            return None
        ...
        if match:
            ...
            ros_node.publish_console_log(
                f"[ERROR] Parameter '{ROS_PARAM_NAME}' is not set on node '{NODE_NAME}'.",
                vehicle,
            )
        else:
            ros_node.publish_console_log(
                f"[ERROR] Failed to retrieve parameter: {output}", vehicle
            )

    except Exception as e:
        ros_node.publish_console_log(
            f"[ERROR] Exception while getting ROS param: {e}", vehicle
        )
    finally:
        ssh.close()
...
def update_yaml_param(file_path, param_name, value, node_name, namespace, vehicle):
    ...
    ros_node.publish_console_log(
        f"[INFO] Updated {file_path} with {param_name} = {value} under {namespace}/{node_name}",
        vehicle,
    )
