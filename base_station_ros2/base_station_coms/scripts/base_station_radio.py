...
class RFBridge(Node):
    def __init__(self):
        ...
        self.vehicles_in_mission = (
            self.get_parameter("vehicles_in_mission")
            .get_parameter_value()
            .integer_array_value
        )
        ...
        self.ping_frequency = (
            self.get_parameter("ping_frequency").get_parameter_value().integer_value
        )

        # base station vehicle ID
        self.declare_parameter("vehicle_id", 15)
        self.vehicle_id = (
            self.get_parameter("vehicle_id").get_parameter_value().integer_value
        )

        # XBee configuration
        self.xbee_port = self.declare_parameter(
            "xbee_port", "/dev/frost/xbee_radio"
        ).value
        ...
        self.rf_connection_publisher = self.create_publisher(
            Connections, "connections", 10
        )

        # publishes console log messages to GUI
        self.print_to_gui_publisher = self.create_publisher(
            ConsoleLog, "console_log", 10
        )

        # Service to send emergency kill command
        self.e_kill_service = self.create_service(
            BeaconId, "radio_e_kill", self.send_e_kill_callback
        )

        # Service to request status from a specific vehicle
        self.status_service = self.create_service(
            BeaconId, "radio_status_request", self.request_status_callback
        )

        self.load_mission_service = self.create_service(
            LoadMission, "radio_load_mission", self.load_mission_callback
        )
        ...
        self.key_controls_subscription = self.create_subscription(
            UCommandBase, "radio_key_command", self.key_controls_callback, 10
        )
        ...
        self.max_msgs_missed = (
            3  # Number of missed messages before considering a vehicle disconnected
        )
        for vehicle in self.vehicles_in_mission:
            self.connections[vehicle] = False
            self.ping_timestamp[vehicle] = (
                self.get_clock().now().nanoseconds / 1e9
            )  # Initialize with current time in seconds

        self.local_fleet_params_path = Path(
            f"/home/frostlab/base_station/mission_control/params/fleet_params.yaml"
        )
        self.local_vehicle_params_path = Path(
            f"/home/frostlab/base_station/mission_control/params/"
        )
            ...
            if message_type == "STATUS":
                ...
                self.print_to_gui_publisher.publish(
                    ConsoleLog(
                        ...
                        vehicle_number=data.get("src_id"),
                    )
                )
            elif message_type == "FILE_ACK":
                # Handle file transfer acknowledgments
                self.get_logger().debug(
                    f"Received file transfer ACK from vehicle {data.get('src_id')}: {data.get('status', 'unknown')}"
                )
    ...
    def request_status_callback(self, request, response):
        ...
        try:
            ...
            response.success = self.send_message(
                status_request_msg, self.radio_addresses.get(target_vehicle_id, None)
            )
        except Exception as e:
    ...
    def load_mission_callback(self, request, response):
        try:
            ...
            vehicle_params_path = (
                self.local_vehicle_params_path / f"coug{target_vehicle_id}_params.yaml"
            )

            if target_vehicle_id is None:
                self.get_logger().error(
                    "Load mission request missing target vehicle ID."
                )
                response.success = False
                return response
            ...
            if target_vehicle_id not in self.radio_addresses:
                self.get_logger().error(
                    f"No radio address found for vehicle {target_vehicle_id}"
                )
                response.success = False
                return response

            self.get_logger().info(
                f"Loading mission file {mission_file_path} to Coug {target_vehicle_id} via radio"
            )
            ...
            success0 = self.send_file_over_radio(
                mission_file_path, target_vehicle_id, "mission.yaml"
            )
            ...
            success1 = self.send_file_over_radio(
                vehicle_params_path,
                target_vehicle_id,
                f"coug{target_vehicle_id}_params.yaml",
            )
            ...
            success2 = self.send_file_over_radio(
                self.local_fleet_params_path, target_vehicle_id, f"fleet_params.yaml"
            )
            ...
            self.print_to_gui_publisher.publish(
                ConsoleLog(
                    ...
                    vehicle_number=(
                        request.vehicle_id if hasattr(request, "vehicle_id") else 0
                    ),
                )
            )
            response.success = False
    ...
    def send_file_over_radio(self, file_path, target_vehicle_id, remote_filename):
        ...
        try:
            ...
            self.print_to_gui_publisher.publish(
                ConsoleLog(
                    message=f"Sending file {file_path} ({file_size} bytes) in {total_chunks} chunks to vehicle {target_vehicle_id}",
                    vehicle_number=target_vehicle_id,
                )
            )
            ...
            if not self.send_message(
                json.dumps(start_msg), self.radio_addresses[target_vehicle_id]
            ):
            ...
            for chunk_num in range(total_chunks):
                ...
                while retry_count < max_retries:
                    if self.send_message(
                        json.dumps(chunk_msg), self.radio_addresses[target_vehicle_id]
                    ):
                        self.get_logger().debug(
                            f"Sent chunk {chunk_num + 1}/{total_chunks}"
                        )
                        break
                    else:
                        retry_count += 1
                        self.get_logger().warn(
                            f"Failed to send chunk {chunk_num + 1}/{total_chunks}, retry {retry_count}/{max_retries}"
                        )
                        time.sleep(0.1)

                if retry_count >= max_retries:
                    self.get_logger().error(
                        f"Failed to send chunk {chunk_num + 1}/{total_chunks} after {max_retries} retries"
                    )
                    return False

                self.print_to_gui_publisher.publish(
                    ConsoleLog(
                        message=f"Sent chunk {chunk_num + 1}/{total_chunks}",
                        vehicle_number=target_vehicle_id,
                    )
                )
            ...
            if not self.send_message(
                json.dumps(end_msg), self.radio_addresses[target_vehicle_id]
            ):
                self.get_logger().error("Failed to send FILE_END message")
                return False

            self.get_logger().info(
                f"Successfully sent file {remote_filename} to vehicle {target_vehicle_id}"
            )
            return True
    ...
    def recieve_status(self, data):
        ...
        status.safety_status.emergency_status = self.make_int8(
            safety_status.get("e_s", 0)
        )
    ...
    def recieve_ping(self, sender_id, sender_address):
        ...
        if sender_id not in self.vehicles_in_mission:
            self.get_logger().warn(
                f"Received PING from unknown vehicle ID {sender_id}. Ignoring."
            )
            return
    ...
    def init_callback(self, request, response):
        try:
            ...
            if target_vehicle_id is None:
                self.get_logger().error(
                    "Initialization request missing target vehicle ID."
                )
                response.success = False
                return response

            self.get_logger().debug(
                f"Received initialization request for Coug {target_vehicle_id}"
            )
            ...
            self.send_message(
                json.dumps(init_msg), self.radio_addresses.get(target_vehicle_id, None)
            )
            response.success = True
        except Exception as e:
    ...
    def send_e_kill_callback(self, request, response):
        try:
            ...
            if target_vehicle_id is None:
                self.get_logger().error(
                    "Emergency kill request missing target vehicle ID."
                )
                response.success = False
                return response

            self.get_logger().debug(
                f"Received emergency kill request for Coug {target_vehicle_id}"
            )

            e_kill_msg = "E_KILL"

            self.send_message(
                e_kill_msg, self.radio_addresses.get(target_vehicle_id, None)
            )
            response.success = True
        except Exception as e:
    ...
    def confirm_e_kill(self, data):
        self.get_logger().info(
            f"Confirmation emergency kill for Coug {data.get('src_id')} was {'successful' if data.get('success') else 'unsuccessful'}"
        )
        if data.get("success"):
            self.print_to_gui_publisher.publish(
                ConsoleLog(
    ...
    def key_controls_callback(self, msg):
        ...
        if vehicle_id not in self.vehicles_in_mission:
            self.get_logger().warn(
                f"Received keyboard controls for unknown vehicle ID {vehicle_id}. Ignoring."
            )
            return
        if self.connections.get(vehicle_id, False):
            self.get_logger().info(
                f"Sending keyboard controls to Coug {vehicle_id} through radio"
            )
            key_msg = {
                "message": "KEY_CONTROL",
                "command": {
            ...
            self.send_message(
                json.dumps(key_msg), self.radio_addresses.get(vehicle_id, None)
            )
        else:
            self.get_logger().warn(
                f"Cannot send keyboard controls to Coug {vehicle_id} because there is no connection"
            )
