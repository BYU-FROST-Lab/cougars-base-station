...
public:
  ModemComs() : Node("base_station_modem") {
    ...
    // id of the base station beacon, used to identify the base station in the
    // network
    ...
    this->modem_subscriber_ =
        this->create_subscription<seatrac_interfaces::msg::ModemRec>(
            "modem_rec", 10, std::bind(&ModemComs::listen_to_modem, this, _1));
    ...
    // service that sends an emergency kill command to a vehicle specified in
    // the request
    ...
    // service that sends an emergency surface command to a vehicle specified in
    // the request
    ...
    init_service_ = this->create_service<base_station_interfaces::srv::Init>(
        "modem_init", std::bind(&ModemComs::init_callback, this, _1, _2));

    // publisher for the status of the cougs, published to the status topic
    this->status_publisher_ =
        this->create_publisher<base_station_interfaces::msg::Status>("status",
                                                                     10);

    // publisher for the connections of the vehicles in the mission via modem
    this->modem_connections_publisher_ =
        this->create_publisher<base_station_interfaces::msg::Connections>(
            "connections", 10);

    // publisher for the confirmation of the emergency kill command
    this->print_to_gui_pub =
        this->create_publisher<base_station_interfaces::msg::ConsoleLog>(
            "console_log", 10);
    ...
    this->timer_ = this->create_wall_timer(
        ...
        std::bind(&ModemComs::check_modem_connections, this));
  ...
  void emergency_kill_callback(
    ...
    send_acoustic_message(request->beacon_id, sizeof(e_kill_msg),
                          (uint8_t *)&e_kill_msg, MSG_OWAY);
    // RCLCPP_INFO(rclcpp::get_logger("rclcpp"), "Emergency Kill Signal Sent to
    // Coug %i", request->beacon_id);
  ...
  void emergency_surface_callback(
    ...
    send_acoustic_message(request->beacon_id, sizeof(e_surface_msg),
                          (uint8_t *)&e_surface_msg, MSG_OWAY);
    // RCLCPP_INFO(rclcpp::get_logger("rclcpp"), "Emergency Surface Signal Sent
    // to Coug %i", request->beacon_id);
    response->success = true;
  }

  // requests status of coug specified in request
  void status_request_callback(
      const std::shared_ptr<base_station_interfaces::srv::BeaconId::Request>
          request,
      std::shared_ptr<base_station_interfaces::srv::BeaconId::Response>
          response) {
    RCLCPP_DEBUG(rclcpp::get_logger("rclcpp"), "Requesting Status of Coug %i",
                 request->beacon_id);
    this->messages_missed_[request->beacon_id]++;

    RequestStatus request_status_msg;
    send_acoustic_message(request->beacon_id, sizeof(request_status_msg),
                          (uint8_t *)&request_status_msg, MSG_OWAY);
    // RCLCPP_INFO(rclcpp::get_logger("rclcpp"), "Requesting Status of Coug %i",
    // request->beacon_id);
    response->success = true;
  }
  ...
  void init_callback(
    ...
    RCLCPP_INFO(rclcpp::get_logger("rclcpp"),
                "Init command received for Coug %i", request->vehicle_id);

    Init init_msg;
    init_msg.init_bitmask = (request->start.data ? 0x01 : 0x00) |
                            (request->rosbag_flag.data ? 0x02 : 0x00) |
                            (request->thruster_arm.data ? 0x04 : 0x00) |
                            (request->dvl_acoustics.data ? 0x08 : 0x00);
    strncpy(init_msg.rosbag_prefix, request->rosbag_prefix.c_str(),
            sizeof(init_msg.rosbag_prefix) - 1);
    init_msg.rosbag_prefix[sizeof(init_msg.rosbag_prefix) - 1] =
        '\0'; // Ensure null termination

    send_acoustic_message(request->vehicle_id, sizeof(init_msg),
                          (uint8_t *)&init_msg, MSG_OWAY);
    response->success = true;
  }
  ...
  void recieve_status(seatrac_interfaces::msg::ModemRec *msg) {
    ...
    const VehicleStatus *status =
        reinterpret_cast<const VehicleStatus *>(msg->packet_data.data());
    ...
    status_msg.safety_status.depth_status.data =
        (status->safety_mask & 0x01) != 0;
    status_msg.safety_status.gps_status.data =
        (status->safety_mask & 0x02) != 0;
    status_msg.safety_status.modem_status.data =
        (status->safety_mask & 0x04) != 0;
    status_msg.safety_status.dvl_status.data =
        (status->safety_mask & 0x08) != 0;
    status_msg.safety_status.emergency_status.data =
        (status->safety_mask & 0x10) != 0;
    ...
    RCLCPP_INFO(this->get_logger(), "position (x, y, z): (%.2f, %.2f, %.2f)",
                static_cast<double>(status->x), static_cast<double>(status->y),
    ...
    RCLCPP_INFO(this->get_logger(), "battery voltage: %.2f",
                static_cast<double>(status->battery_voltage));
    RCLCPP_INFO(this->get_logger(), "battery percentage: %.2f",
                static_cast<double>(status->battery_percentage));
    RCLCPP_INFO(this->get_logger(), "pressure: %.2f",
                static_cast<double>(status->pressure));
  }
  ...
  void emergency_kill_confirmed(seatrac_interfaces::msg::ModemRec *msg) {
    ...
    if (msg->packet_data[1]) {
      message = "Emergency kill command was successful for Coug " +
                std::to_string(msg->src_id);
    } else {
      message = "Emergency kill command failed for Coug " +
                std::to_string(msg->src_id);
    }
  ...
  void emergency_surface_confirmed(seatrac_interfaces::msg::ModemRec *msg) {
    ...
    if (msg->packet_data[1]) {
      message = "Emergency surface command was successful for Coug " +
                std::to_string(msg->src_id);
    } else {
      message = "Emergency surface command failed for Coug " +
                std::to_string(msg->src_id);
    }
    base_station_interfaces::msg::ConsoleLog log_msg;
    log_msg.message = message;
  ...
  // checks the connections of the vehicles in the mission and publishes the
  // connections
    ...
    for (auto id : this->vehicles_in_mission_) {
      if (this->messages_missed_[id] >= this->max_missed_messages) {
        if (this->modem_connection[id]) {
          RCLCPP_WARN(
              this->get_logger(),
              "Coug %i has missed %i or more messages, marking as disconnected",
              id, this->max_missed_messages);
      ...
      msg.last_ping.push_back(static_cast<uint64_t>(
          this->now().seconds() - last_message_time_[id].seconds()));
    }
...
private:
  ...
  rclcpp::Publisher<base_station_interfaces::msg::Status>::SharedPtr
      status_publisher_;
  rclcpp::Publisher<base_station_interfaces::msg::Connections>::SharedPtr
      modem_connections_publisher_;
  rclcpp::Publisher<base_station_interfaces::msg::ConsoleLog>::SharedPtr
      print_to_gui_pub;
  ...
  std::unordered_map<int, int>
      messages_missed_; // keeps track of how many messages have been missed for
                        // each coug
};
