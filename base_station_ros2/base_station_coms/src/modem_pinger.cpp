 ...
 *
 * The ping schedule is based on 3 parameters: ping_delay_seconds,
 * number_of_vehicles, and vehicle_order, and uses the following formula:
 * - ping_cycle_length = ping_delay_seconds * number_of_vehicles
 * - seconds_into_cycle = seconds_since_midnight % ping_cycle_length
 * - time_until_ping = (ping_delay_seconds * vehicle_order - seconds_into_cycle)
 * % ping_cycle_length
 *
...
public:
  SeatracPinger() : Node("modem_pinger") {
     ...
     * at the same time.
     */
    this->declare_parameter<int>("vehicle_order",
                                 -1); //-1 indicates unspecified
     ...
     * @param target_id
     *
     * The vehicle to address this ping to. If target_id is zero then the ping
     * sent will be addressed to all vehicles. If the target_id is between 1 and
     * 16, it will address the message to the vehicle with vehicle_ID=target_id.
     *
     * Acoustic messages are always broadcasted to and received by every
     * vehicle, regardless of which specific vehicle the message is addressed
     * to. However, only the vehicle this message is addressed to will respond
     * with a return message. And so, if request_response is true, this
     * parameter must be set between 1 and 16. If request_response is false, it
     * makes no difference what the target_id is, so you leave it at 0 to send
     * to all beacons.
     */
    ...
    if (this->vehicle_order_ == -1)
      this->vehicle_order_ = this->get_parameter("vehicle_ID").as_int();
...
private:
  ...
  void repeat_call_ping(const std_msgs::msg::Empty::SharedPtr msg) {
    ...
    while (rclcpp::ok()) {
      ...
      int sleep_time =
          (my_ping_second - seconds_in_round + total_seconds_per_round) %
          total_seconds_per_round;
  ...
  void send_ping() {
    ...
    request.msg_type =
        request_response_ ? AMSGTYPE_E::MSG_REQU : AMSGTYPE_E::MSG_OWAYU;
